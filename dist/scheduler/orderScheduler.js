...
import { floorToFiveMinuteBucketUtc, formatIso, toUtcMillis } from "../utils/time.js";
const REQUIRED_SYMBOLS = ["BTC", "ETH", "SOL", "XRP"];
export class OrderScheduler {
    marketDiscoveryService;
    orderService;
    intervalHandle;
    tickInProgress = false;
    maxWalletStartTimeMs = 0;
    currentCycleStartMs;
    lastMissingSymbolsSignature = "";
    cyclePlacedMarkets = new Set();
    constructor(marketDiscoveryService, orderService) {
    ...
    async start() {
        ...
        }, "Starting order scheduler");
        const maxMs = await this.orderService.getMaxWalletOrderStartTimeMs(this.marketDiscoveryService);
        this.maxWalletStartTimeMs = maxMs ?? 0;
        logger.info({ maxWalletStartTimeIso: formatIso(this.maxWalletStartTimeMs) }, "Wallet history order check complete");
        await this.tick();
    ...
    async tick() {
        ...
        try {
            ...
            const unplacedMarkets = marketsForTargetCycle.filter((market) => !this.cyclePlacedMarkets.has(market.conditionId));
            if (unplacedMarkets.length > 0) {
                ...
                if (this.cyclePlacedMarkets.size === REQUIRED_SYMBOLS.length) {
                    ...
                    logger.info({ targetStartTimeIso: formatIso(targetStartTimeMs) }, "Successfully placed all orders for target cycle. Waiting for next live market change snapshot.");
                }
            }
        }
        catch (error) {
    ...
    selectTargetCycleMarkets(discoveredMarkets, targetStartTimeMs) {
...
//# sourceMappingURL=orderScheduler.js.map