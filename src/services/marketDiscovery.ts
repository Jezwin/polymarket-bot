...
import type { CryptoSymbol, DiscoveredMarket, GammaMarket } from "../types/market.js";
...
const getOutcomeIndexes = (outcomes: string[]): { yesIndex: number; noIndex: number } => {
...
const getMarketSearchText = (market: GammaMarket): string => {
...
const detectCryptoSymbol = (market: GammaMarket): CryptoSymbol | null => {
  const event = market.events?.[0];
  const series = event?.series?.[0];

  if (series?.recurrence !== "5m" && series?.recurrence !== "15m") {
    return null;
  }
...
export class MarketDiscoveryService {
  ...
  async discoverMarkets(): Promise<DiscoveredMarket[]> {
    ...
    for (const market of markets) {
      ...
      const series = market.events?.[0]?.series?.[0];
      const recurrence = series?.recurrence || "5m";
    ...
    logger.info({ discovered: discovered.length }, "Discovered qualifying 5/15-minute crypto markets");

    return discovered;
  }

  async detectStartupTargetStartTimeMs(discoveredMarkets: DiscoveredMarket[]): Promise<number> {
    if (discoveredMarkets.length === 0) {
      throw new Error("No qualifying BTC/ETH/SOL/XRP 5/15-minute markets discovered at startup");
    }
  ...
  async getMarketStartTimeByConditionId(conditionId: string): Promise<number | undefined> {
    const markets = await withRetry(
      async () => {
        const response = await this.http.get<GammaMarket[]>("/markets", {
