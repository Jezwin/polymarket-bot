...
export class PolygonWsClient {
    ...
    constructor() {
        this.exchangeAddress = getContractConfig(env.CHAIN_ID as Chain).exchange;
    }
     ...
     */
    public async awaitOrderFill(orderId: string): Promise<void> {
        if (!this.provider) {
        ...
        return new Promise<void>((resolve, reject) => {
            if (!this.provider) return reject("No provider");
